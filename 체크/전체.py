...
def generate(지역, 메인, result_path, out_dir, template, 위치사진='', 링크섹션='', 상위링크='/', dry_run=False):
    """
...
def main():
    ...
    for dong, (do_nm, si_nm, branch, 위치사진) in dong_map.items():
        ...
        if do_nm not in done_lv1:
            ...
            r = generate(do_nm, MAIN_FIXED, r_path, out_dir, template, '', lk, '/', args.dry_run)
            ok += r; skip += not r
        ...
        if (do_nm, si_nm) not in done_lv2:
            ...
            r = generate(si_nm, MAIN_FIXED, r_path, out_dir, template, '', lk, f'/{do_nm}/', args.dry_run)
            ok += r; skip += not r
        ...
        if (do_nm, si_nm, dong) not in done_lv3:
            ...
            r = generate(dong, MAIN_FIXED, r_path, out_dir, template, 위치사진, lk, f'/{do_nm}/{si_nm}/', args.dry_run)
            ok += r; skip += not r
        ...
        for kw in keywords:
            ...
            r = generate(dong, kw, r_path, out_dir, template, 위치사진, '', f'/{do_nm}/{si_nm}/{dong}/', args.dry_run)
            ok += r; skip += not r
