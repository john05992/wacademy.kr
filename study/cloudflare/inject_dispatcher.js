...
(function(){
  ...
  fetch('/study/templates/'+tpl+'.html')
    ...
    .then(function(html){
      ...
      if(/와와/.test(kw)){html=html.split('맞춤학원').join('와와코칭학원');var cm=html.match(/<section[^>]*id="consult"[^>]*>[\\s\\S]*?<\\/section>/);if(cm){var ch='<div id="wawa-hero"><style>#wawa-hero{padding:100px 20px 56px;text-align:center;background:#f8f8f8}#wawa-hero .c3-section{background:transparent!important;padding:0}#wawa-hero .c3-img{display:none}#wawa-hero .c3-warn{color:#111!important}#wawa-hero .c3-num{background:linear-gradient(90deg,#FF4714,#f5af19)!important;color:#fff!important}#wawa-hero .c3-sub{color:#999!important}#wawa-hero::after{content:"";display:block;width:2px;height:72px;background:linear-gradient(to bottom,rgba(255,71,20,.7),transparent);margin:40px auto 0}#wawa-ttl{font-size:26px;font-weight:800;color:#111;margin:0 0 20px;line-height:1.4}</style><p id="wawa-ttl">'+loc+' 와와코칭학원</p>'+cm[0]+'</div>';html=html.replace(/<body[^>]*>/,function(m){return m+ch;});}}
